...
class LetExpr(Expr):
    def __init__(self, name: str, assign_expr: Expr, inner_expr: Expr):
        self._name = name
        self._assign_expr = assign_expr
        self._inner_expr = inner_expr

    def prefix(self) -> str:
        return f"let {self._name} = {self._assign_expr.prefix()} in {self._inner_expr.prefix()}"

    def infix(self) -> str:
        return f"(let {self._name} = {self._assign_expr.infix()} in {self._inner_expr.infix()})"

    def eval(self, context: dict[str, float]) -> float:
        # Create a new context for evaluation to avoid affecting the outer scope
        new_context = dict(context)  # Copy the existing context
        new_context[self._name] = self._assign_expr.eval(context)
